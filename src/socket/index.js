import { saveMessage, saveMessage2 } from '../utils/messages.js'
...
const connectionHandler = (socket) => {
  ...
  socket.on('message', async (sender, content, room) => {
    ...
    const foundRoom = await RoomModel.find({ room })
    console.log('FoundRoom', foundRoom)
    if (foundRoom.length > 0) {
      await saveMessage(sender, content, room) // when full
    } else {
      await saveMessage2(sender, content, room) // when empty
    }
